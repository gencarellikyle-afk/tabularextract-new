...
- For merged cells: place full text in LEFTMOST column ONLY, leave right columns blank or repeat parent.
...
def final_polish(df):
    new_cols = [re.sub(r'Column header \(TH\)|Row header \(TH\)|Data cell \(TD\)|\(TH\)|\(TD\)|Unnamed: \d+|Column_\d+|\.\d+', '', str(col).strip(), flags=re.IGNORECASE) or f"Column_{i}" for i, col in enumerate(df.columns)]
    df.columns = new_cols
        ...
        for i, t in enumerate(tables_list):
            ...
            if any(x in raw_csv for x in ["(TH)", "(TD)", "Column header", "Row header"]):
                resp = client.messages.create(
                    ...
                    max_tokens=4000,
                    temperature=0.0,
                    system=PERFECTION_PROMPT + "\nThis table contains extreme placeholders. Strip EVERY (TH) and (TD) reference and create clean column names.",
